...
int get_decimal(char *token){
    // This function will return the decimal value of the token, either it is hexadecimal or decimal 
    char *hex = "0x";
...
char *trim(char *ins){
    // This function will trim the string ins, and return the pointer to the first non-space character
...
int standardize_integer(int number){
    // This function will standardize the integer to 32 bits
    return number % 0x1000;
}

int sign_extend_i(int number){
    // This function will sign extend the number to 32 bits
...
void step(char *instruction)
{
    ...
    if ((op_type == R_TYPE) || (op_type == I_TYPE)){

        if (des_reg == 0){
            // we cannot write to x0, so we skip this instruction
            return; 
        }
        ...
        if (op_type == R_TYPE){
            ...
            // Branch to the corresponding operation
            if (strcmp(op, "add") == 0){
        ...
        if (op_type == I_TYPE){
            reg_2_imm = standardize_integer(reg_2_imm);
            reg_2_imm = sign_extend_i(reg_2_imm);
            if (strcmp(op, "addi") == 0){
                registers->r[des_reg] = registers->r[reg_1] + reg_2_imm;
            }
